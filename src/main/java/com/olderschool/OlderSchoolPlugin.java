...
public class OlderSchoolPlugin extends Plugin {
	...
	// Greater Demon
	private static final Set<Integer> GREATER_DEMON_SOURCE_IDS = Set.of(7244, 7245, 7246, 2025, 2026, 2027, 2028, 2029);
	...
	// Black Demon
	private static final Set<Integer> BLACK_DEMON_SOURCE_IDS = Set.of(7242, 7243, 2048, 2049, 2050, 2051, 2052);
	...
	// Stores original compositions keyed by NPC identity hash
	private final Map<Integer, NPCComposition> originalComps = new HashMap<>();
	// Stores original idle/walk anims keyed by NPC identity hash: int[]{idle, walk}
			...
			if (currentComp != null && currentComp != targetComp) {
				...
				if (compField != null) {
					try {
						...
						System.out.println("Comp swap failed: " + e.getMessage());
					}
				}
			}
